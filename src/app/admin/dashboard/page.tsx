            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-3 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500 sm:px-4">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200 bg-white">
                {loading ? (
                  ...
                  requests.map((request) => (
                    <tr key={request.id}>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-900 sm:px-4">
                        {request.name || "—"}
                      </td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-900 sm:px-4">
                        {request.service || "—"}
                      </td>
                      <td className="max-w-xs px-3 py-4 text-sm text-gray-700 sm:px-4">
                        <div className="truncate">{request.message || "—"}</div>
                      </td>
                      <td className="whitespace-nowrap px-3 py-4 text-xs text-gray-500 sm:px-4 sm:text-sm">
                        {request.timestamp
